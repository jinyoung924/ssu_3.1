...
engine = create_engine('mysql+pymysql://root:root@localhost/restaurant') #-- 프로젝트마다 ''부분만 바뛴다
...
DBSession = sessionmaker(bind=engine) #-- 프로세스마다 인스턴스로 session을 만든다.]
...
#-- 이 위에 부분은 연결해주는 부분이 이니깐 복붙만 하면 되고 밑에 부분은 sql문을 python으로 처리해주는 부분이다.
