...
params0_name_to_update = [] #-- layer3의 파라미터 넣는 곳 !!새로 작성!!
...
for name, param in model.named_parameters():
    if 'layer3' in name:                            #-- layer3도 학습되도록 수정한 부분!!
        param.requires_grad = True  
        params0_to_update.append(param)
        params0_name_to_update.append(name)
    elif 'layer4' in name:                          #-- 여기에서는 fc층 직전에 있는 feature층 (layer4) '하나'만 학습을 시켜보겠다.
...
optim = Adam([{'params': params0_to_update, 'lr': 1e-6},  # layer3 학습을 위해서 옵티마이저에도 추가해 줘야한다!!
